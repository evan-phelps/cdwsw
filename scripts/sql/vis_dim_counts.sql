     ...
     HSSC_ETL.DROP_TABLE_IF_EXIST(m_table, 'HSSC_ETL');
      ...
      FOR it IN 1 .. m_vis_attrib.count LOOP
        DBMS_OUTPUT.PUT_LINE('Attr/Val : ' ||
          m_vis_attrib(it) || ' / ' || m_vis_value(it) );
        
        /* for site projects the value (column) to look up
           has a suffix _ORIG if the site is not HSSC
           also fin_class_group morphs into fin_class_orig
        */
        m_value := m_vis_value(it);
        IF ( substr(m_meta,5,(LENGTH(m_meta)-8)) != 'HSSC') THEN
            IF (m_value = 'fin_class_group') THEN
              m_value := 'fin_class';
            ELSIF (m_value = 'hosp_srv_group') THEN
              m_value := 'hosp_srv';
            END IF;
            m_value := m_value || '_ORIG';
        END IF;

        t_query := 'INSERT INTO ' || m_table || ' (SELECT  ''' ||
            m_vis_attrib(it) || ''' attr, v.' || m_value
          ...
          like ''' || m_vis_attrib(it) || ':%'' ) ov full outer join
            ( select ' || m_value || ', count('
            || m_value || ') n_attrs, count(distinct patient_num) precs,
            count(1) n_recs from ' || m_data || '.VISIT_DIMENSION group by '
                || m_value || ') v on
                   (ov.c_dimcode = v.' || m_value || '))';
